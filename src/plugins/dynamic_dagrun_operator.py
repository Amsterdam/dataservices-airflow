...
class TriggerDynamicDagRunOperator(TriggerDagRunOperator):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            labels_group: Set of labels representing dataset that are grouped
    ...
    def execute(self, context: dict[str, Any]) -> None:
        ...
        self.log.info("{SLACK_ICON_START} Starting dag %s", current_dag_id)
        session = Session()
        query = (
        ...
        if self.labels_group is not None:
            query = query.filter(DagModel.tags.any(DagTag.name.in_(self.labels_group)))

        active_dag_ids = [d.dag_id for d in query.order_by("dag_id")]
