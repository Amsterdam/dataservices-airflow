...
DAG_ID: Final = "touringcars_az"
DATASET_ID: Final = "touringcars"
...
with DAG(
    ...
    provenance_translation = ProvenanceRenameOperator(
        task_id="rename_columns", dataset_name=DATASET_ID, pg_schema="public"
    )
    ...
    drop_tables = [
        PostgresOnAzureOperator(
            ...
            sql=[
                f"DROP TABLE IF EXISTS {DATASET_ID}_{file_name} CASCADE",
            ],
        )
        for file_name in data_endpoints.keys()
    ]
    ...
    rename_tables = [
        PostgresTableRenameOperator(
            ...
            new_table_name=f"{DATASET_ID}_{file_name}",
        )
        for file_name in data_endpoints.keys()
    ]
