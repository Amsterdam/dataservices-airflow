...
DAG_ID: Final = "grootstedelijke_projecten_az"
DATASET_ID: Final = "grootstedelijke_projecten"
DATASET = "gebieden"
# owner: Only needed for CloudVPS. On Azure
# each team has its own Airflow instance.
owner = 'team_benk'
DB_TABLE_NAME = f"{DATASET}_{DATASET_ID}"
TMP_PATH = f"{SHARED_DIR}/{DATASET_ID}"
variables_aardgasvrijezones = Variable.get(DATASET_ID, deserialize_json=True)
...
with DAG(
    ...
    check_name[DATASET_ID] = total_checks

    # 7. Execute bundled checks on database
    multi_checks = PostgresMultiCheckOperator(task_id="multi_check", checks=check_name[DATASET_ID])
