...
DAG_ID: Final = "parkeerzones_az"
DATASET_ID: Final = "parkeerzones"
...
with DAG(
    ...
    SHP_to_SQL = [
        BashOperator(
            ...
            f"-nln {DATASET_ID}_{subject}_new "
            f"{tmp_dir}/{DATASET_ID}_{subject}.sql {tmp_dir}/20190606_Vergunninggebieden/{shp_file} "
    ...
    convert_to_UTF8 = [
        BashOperator(
            ...
            bash_command=f"iconv -f iso-8859-1 -t utf-8  {tmp_dir}/{DATASET_ID}_{subject}.sql > "
            f"{tmp_dir}/UTF8_{DATASET_ID}_{subject}.sql",
        )
    ...
    load_tables = [
        BashOperator(
            ...
            bash_command=f"psql {db_conn_string()} < {tmp_dir}/UTF8_{DATASET_ID}_{subject}.sql",
        )
        for subject in files_to_proces.keys()
    ]
    ...
    revalidate_geometry_records = [
        PostgresOnAzureOperator(
            ...
            params={"tablename": f"{DATASET_ID}_{subject}_new"},
        )
        for subject in files_to_proces.keys()
    ]
    ...
    for subject in files_to_proces.keys():
        ...
        count_checks.append(
            COUNT_CHECK.make_check(
                ...
                pass_value=2,
                params={"table_name": f"{DATASET_ID}_{subject}_new"},
        ...
        geo_checks.append(
            GEO_CHECK.make_check(
                ...
                params={
                    "table_name": f"{DATASET_ID}_{subject}_new",
    ...
    rename_tables = [
        PostgresTableRenameOperator(
            ...
            old_table_name=f"{DATASET_ID}_{subject}_new",
            new_table_name=f"{DATASET_ID}_{subject}",
        )
        for subject in files_to_proces.keys()
    ]
