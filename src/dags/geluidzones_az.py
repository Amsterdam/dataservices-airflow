...
DAG_ID: Final = "geluidszones_az"
DATASET_ID: Final = "geluidszones"
...
with DAG(
    ...
    csv_to_SQL = [
        BashOperator(
            ...
            "-t_srs EPSG:28992 "
            f"-nln {DATASET_ID}_{splitted_tablename}_new "
    ...
    redefine_geoms = [
        PostgresOnAzureOperator(
            ...
            sql=SET_GEOM,
            params=dict(tablename=f"{DATASET_ID}_{splitted_tablename}_new"),
        )
        for key in files_to_download.keys()
        for splitted_tablename in key.split("-")
    ...
    add_thema_contexts = [
        PostgresOnAzureOperator(
            ...
            params=dict(
                tablename=f"{DATASET_ID}_{splitted_tablename}_new", parent_table=f"{DATASET_ID}_themas_new"
            ),
        )
        for key in files_to_download.keys()
        for splitted_tablename in key.split("-")
            ...
            if "themas" not in key:
                ...
                count_checks.append(
                    COUNT_CHECK.make_check(
                        ...
                        pass_value=2,
                        params=dict(table_name=f"{DATASET_ID}_{splitted_tablename}_new"),
                ...
                geo_checks.append(
                    GEO_CHECK.make_check(
                        ...
                        params=dict(
                            table_name=f"{DATASET_ID}_{splitted_tablename}_new",
    ...
    rename_tables = [
        PostgresTableRenameOperator(
            ...
            old_table_name=f"{DATASET_ID}_{splitted_tablename}_new",
            new_table_name=f"{DATASET_ID}_{splitted_tablename}",
        )
        for key in files_to_download.keys()
        for splitted_tablename in key.split("-")
        if "themas" not in key
    ...
    drop_cols = [
        PostgresOnAzureOperator(
            ...
            sql=DROP_COLS,
            params=dict(tablename=f"{DATASET_ID}_{key}"),
        )
