...
variables_overlastgebieden: Dict = Variable.get("overlastgebieden", deserialize_json=True)
files_to_download: Dict = variables_overlastgebieden["files_to_download"]
tables_to_create: Dict = variables_overlastgebieden["tables_to_create"]
# Note: Vuurwerkvrijezones (VVZ) data is temporaly! not processed due to covid19 national measures
tables_to_check: Dict = {k: v for k, v in tables_to_create.items() if k != "vuurwerkvrij"}
TMP_PATH: str = f"{SHARED_DIR}/{dag_id}/"
total_checks: list = []
count_checks: list = []
geo_checks: list = []
check_name: Dict = {}
...
with DAG(
    ...
    mkdir = BashOperator(task_id="mkdir", bash_command=f"mkdir -p {TMP_PATH}")
