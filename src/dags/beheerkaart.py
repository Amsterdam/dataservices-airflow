...
with DAG(dag_id, default_args={**default_args, **{"owner": owner}}) as dag:
    ...
    drop_tables = ProvenanceDropFromSchemaOperator(
        task_id="drop_tables", dataset_name="beheerkaart_basis", pg_schema="pte",
    )
    ...
    swift_load_task = SwiftLoadSqlOperator(
        ...
        object_id=f"beheerkaart_pr/beheerkaart_basis/{DATASTORE_TYPE}/bkt.zip",
    ...
    provenance_renames = ProvenanceRenameOperator(
        ...
        dataset_name="beheerkaart_basis",
        pg_schema="pte",
        rename_indexes=True,
    )
    ...
    swap_schema = SwapSchemaOperator(
        task_id="swap_schema", dataset_name="beheerkaart_basis"
    )
