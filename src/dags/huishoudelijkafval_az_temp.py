...
DAG_ID: Final = "huishoudelijkafval_az_temp"
DATASET_ID: Final = "huishoudelijkafval"
...
with DAG(
    ...
    swap_schema = SwapSchemaOperator(
        task_id="swap_schema", subset_tables=tables["dump_file"], dataset_name=DATASET_ID
    )
    ...
    load_dwh = PythonOperator(
        ...
        op_args=[f"{DATASET_ID}_{to_snake_case(tables['dwh_stadsdelen'])}_new", DATASET_ID],
    )
    ...
    check_count = PostgresCheckOperator(
        ...
        sql=SQL_CHECK_COUNT,
        params={
            "tablename": f"{DATASET_ID}_{to_snake_case(tables['dwh_stadsdelen'])}_new",
            "mincount": 1000,
        },
    )
    ...
    create_tables = SqlAlchemyCreateObjectOperator(
        ...
        data_table_name=f"{DATASET_ID}_{tables['dwh_stadsdelen']}",
        ind_table=True,
    ...
    change_data_capture = PostgresTableCopyOperator(
        ...
        source_table_name=f"{DATASET_ID}_{to_snake_case(tables['dwh_stadsdelen'])}_new",
        target_table_name=f"{DATASET_ID}_{to_snake_case(tables['dwh_stadsdelen'])}",
    ...
    clean_up = PostgresOnAzureOperator(
        ...
        params={"tablename": f"{DATASET_ID}_{to_snake_case(tables['dwh_stadsdelen'])}_new"},
    )
