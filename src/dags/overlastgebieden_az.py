...
DAG_ID: Final = "overlastgebieden_az"
DATASET_ID: Final = "overlastgebieden"
...
TMP_PATH: Final = Path(SHARED_DIR) / DATASET_ID
SOURCE_PATH: Final = Path('Shapes_outbox')
tmp_database_schema: str = define_temp_db_schema(dataset_name=DATASET_ID)
...
with DAG(
    ...
    drop_if_exists_tmp_tables = [
        PostgresOnAzureOperator(
            ...
            sql=SQL_DROP_TABLE,
            params={"schema": tmp_database_schema, "tablename": f"{DATASET_ID}_{key}_new"},
        )
    ...
    remove_null_geometry_records = [
        PostgresOnAzureOperator(
            ...
            params={
                ...
                "tablename": f"{DATASET_ID}_{key}_new",
                "geo_column": "geometry",
                "geom_type_number": "3",
            },
        )
                ...
                COUNT_CHECK.make_check(
                    ...
                    params={
                        "table_name": "{tmp_schema}.{dataset}_{table}_new".format(
                            tmp_schema=tmp_database_schema, dataset=DATASET_ID, table=key
                        )
                    },
    ...
    multi_checks = [
        PostgresMultiCheckOperator(
            task_id=f"multi_check_{key}", checks=check_name[key], dataset_name=DATASET_ID
        )
        for key in tables_to_check.keys()
    ]
...
provenance_translation >> multi_checks >> Interface2 >> copy_data_to_target >> grant_db_permissions
