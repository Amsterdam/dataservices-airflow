...
with DAG(
    ...
    # 5. Dummy operator acts as an Interface between parallel tasks to another parallel tasks (i.e. lists or tuples) 
    ...
    to_sql = [
        BashOperator(
            ...
            f"{tmp_dir}/{file_name}.sql {tmp_dir}/{file_name}.{re.sub('_.*', '', source_type)} "
            f"-lco SEPARATOR=COMMA "
    ...
    # 7. Convert SQL to store all karakters between the code points 128 - 255 (wich are different in iso-8859-1) in UTF-8
    convert_to_UTF8 = [         
        BashOperator(
        task_id=f"convert_to_utf8_{file_name}",
        bash_command=f"iconv -f iso-8859-1 -t utf-8  {tmp_dir}/{file_name}.sql > "
        f"{tmp_dir}/{file_name}.sql.utf8.sql",
    )
    ...
    load_tables = [
        BashOperator(
            ...
            bash_command=f"psql {pg_params()} < {tmp_dir}/{file_name}.sql.utf8.sql",
        )
    ...
    for add_geometry in add_geom_col:
        ...
        add_geometry >> delete_duplicate_rows >> provenance_translation >> multi_checks
