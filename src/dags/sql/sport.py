...
# Before creating the column geometry, there is a check if a X and Y column exists, so it can be used for the geometry.
# If no X and Y exists, create the geometry anyway so it can be filled when spatial data is available.
# Also een check is done on the existince of a value in X and Y (data may contain empty values).
# Also a check is done on the presence of invalid karakter values in X and Y (data may contain letters!).
