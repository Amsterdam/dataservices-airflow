...
# Casting to ::bigint is used to prevent an error when there is null value present and the
...
ALTER TABLE {{ params.tablename }} ALTER COLUMN storing_datum_gemeld TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(storing_datum_gemeld::bigint/1000);
ALTER TABLE {{ params.tablename }} ALTER COLUMN storing_datum_schatting TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(storing_datum_schatting::bigint/1000);
ALTER TABLE {{ params.tablename }} ALTER COLUMN storing_datum_eind TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(storing_datum_eind::bigint/1000);
ALTER TABLE {{ params.tablename }} ALTER COLUMN date_end TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(date_end::bigint/1000);
ALTER TABLE {{ params.tablename }} ALTER COLUMN storing_datum_change TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(storing_datum_change::bigint/1000);
ALTER TABLE {{ params.tablename }} ALTER COLUMN storing_service_update TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(storing_service_update::bigint/1000);
ALTER TABLE {{ params.tablename }} ALTER COLUMN log_date TYPE TIMESTAMP
    WITH TIME zone using TO_TIMESTAMP(log_date::bigint/1000);
COMMIT;
"""
