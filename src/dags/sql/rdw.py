...
encode(md5(
    b.kenteken ||
    coalesce(a.as_nummer, 0) ||
    coalesce(br.brandstof_omschrijving, 'NA')
    )::text::bytea,
    'hex') as id,
b.kenteken,
...
LEFT JOIN rdw_assen_new a
on b.kenteken = a.kenteken
LEFT JOIN rdw_brandstof_new br
on b.kenteken = br.kenteken
);
...
ALTER TABLE IF EXISTS {{ params.tablename }}_new RENAME TO {{ params.tablename }};
ALTER INDEX IF EXISTS {{ params.tablename }}_new_idx RENAME TO {{ params.tablename }}_idx;
ALTER TABLE IF EXISTS {{ params.tablename }} RENAME CONSTRAINT {{ params.tablename }}_new_pk
