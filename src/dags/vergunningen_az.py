...
os.environ["AIRFLOW_CONN_POSTGRES_DEFAULT"] = os.environ["AIRFLOW_CONN_POSTGRES_AZURE_OPDR"]

DAG_ID: Final = "vergunningen_az"
DATASET_ID: Final = "vergunningen"
...
tmp_dir = f"{EPHEMERAL_DIR}/{DATASET_ID}"
...
with DAG(
    ...
    replace_tablename = [
        BashOperator(
            ...
            bash_command=f'perl -pi -e "s/{source_name}/{DATASET_ID}_{target_name}_new/g" '
            f"{tmp_dir}/{file}",
        )
    ...
    create_tables = [
        SqlAlchemyCreateObjectOperator(
            ...
            data_table_name=f"{DATASET_ID}_{target_name}",
            ind_table=True,
    ...
    recreate_tmp_tables = [
        PostgresOnAzureOperator(
            ...
            params={"tablename": f"{DATASET_ID}_{target_name}"},
        )
    ...
    for _, _, target_name in table_renames:
        ...
        count_checks.append(
            COUNT_CHECK.make_check(
                ...
                pass_value=10,
                params={"table_name": f"{DATASET_ID}_{target_name}_new"},
        ...
        geo_checks.append(
            GEO_CHECK.make_check(
                ...
                params={
                    "table_name": f"{DATASET_ID}_{target_name}_new",
                    "geotype": [
                        "MULTIPOLYGON",
                    ],
    ...
    change_data_capture = [
        PostgresTableCopyOperator(
            ...
            source_table_name=f"{DATASET_ID}_{target_name}_new",
            target_table_name=f"{DATASET_ID}_{target_name}",
